...
//Shader2o - A shader for drawing tinted 2d textured meshes
Shader2p::Shader2p() : Shader("res/shaders/s2p/vert.vs", "res/shaders/s2p/frag.fs")
{
	getUniformLocs();
}

void Shader2p::getUniformLocs() {
	distanceLoc = glGetUniformLocation(program, "distance");
}

void Shader2p::setDistance(float distance)
{
